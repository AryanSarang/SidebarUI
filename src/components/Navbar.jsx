import React from "react";
import PageContent from "./PageContent";
import Sidebar from "./Sidebar";
import MenuOpenIcon from '@mui/icons-material/MenuOpen';
import NotificationsIcon from '@mui/icons-material/Notifications';
import HomeIcon from '@mui/icons-material/Home';
const Navbar = () => {
  return (
    <div>
      <div className="d-flex" id="wrapper">
        {/* <!-- Sidebar--> */}
        <Sidebar />
        {/* <!-- Page content wrapper--> */}
        <div id="page-content-wrapper" >
          {/* <!-- Top navigation--> */}
          <nav className="navbar navbar-expand-lg navbar-light bg-light border-bottom">
            <div className="container-fluid leftOfNav">

              <button className="btn" id="sidebarToggle" type="button" data-bs-toggle="collapse"
                data-bs-target="#sidebar-wrapper" aria-expanded="false" aria-controls="sidebar-wrapper"><MenuOpenIcon/></button>

              <button className="navbar-toggler" type="button" data-bs-toggle="collapse"
                data-bs-target="#navbarSupportedContent" aria-controls="navbarSupportedContent"
                aria-expanded="false" aria-label="Toggle navigation"><span className="navbar-toggler-icon"></span></button>
              <div className="collapse navbar-collapse" id="navbarSupportedContent">
                <ul className="navbar-nav ms-auto mt-2 mt-lg-0">
                  <li className="nav-item active"><a className="nav-link" href="#!"><HomeIcon/></a></li>
                  <li className="nav-item"><a className="nav-link" href="#!"><NotificationsIcon/></a></li>
                  <li className="nav-item dropdown">
                    <button className="btn btn-light nav-link dropdown-toggle" id="navbarDropdown" type="button"
                      data-bs-toggle="dropdown" aria-expanded="false"><img src="data:image/jpeg;base64," alt="profile" width="30" height="30" className="rounded-circle me-2" />
                      Jack Sparrow</button>
                    <ul className="dropdown-menu dropdown-menu-lg-end">
                      <li> <a className="dropdown-item" href="#">Action</a></li>
                      <li><a className="dropdown-item" href="#">Another action</a></li>
                      <div className="dropdown-divider"></div>
                      <li><a className="dropdown-item" href="#">Log out</a></li>
                    </ul>
                  </li>
                </ul>
              </div>
            </div>
          </nav>
          {/* <!-- Page content--> */}
          <PageContent />
        </div>
      </div>
    </div>
  );

}

export default Navbar;